...
enum fnd_mode  { FND_UserButton, FND_AdcValue, FND_Temper, FND_Date, FND_KeySw, FND_PigGame, FND_ECHO, FND_End } FND_MODE = FND_KeySw;
enum clcd_mode { AUTO_CMODE, BAR_CMODE, ADC_CMODE, TIME_CMODE, PIG_CMODE, ECHO_CMODE } CLCD_MODE = AUTO_CMODE;
//
//
...
int main(void)
{
	...
	//
	EVSYS.CHANNEL0 = EVSYS_GENERATOR_TCB3_CAPT_gc;
	...
	FndTest();  
	if ( EEPROM_ReadUint16( 40 ) != 0x5a5a ) {
		IOX_CLCD_DisplayString( 0, 0, "Hello ATMEGA4809");
		_delay_ms(1200);
		EEPROM_WriteUint16( 40, 0x5a5a );		// 0b01011010
	} else {
		IOX_CLCD_DisplayString( 0, 1, "Hello again!!");
		btValue = EEPROM_ReadUint16( 40 );
		_delay_ms(500);
	}
    ...
    while (1) 
    {
		...
		if ( gBtFlag ) {
			gBtFlag = false;
			if ( !EncoderEnable ) { 
				btValue++;	EEPROM_WriteUint16( 40, btValue );
		...
		if ( gkswFlag ) {
			...
			if ( temp < 10 ) {	// Numeric key
				...
				switch ( temp ) {
					case 'B' :
						sprintf( buff1, "Enter Rev No?:%3d", NumberValue );  IOX_CLCD_DisplayString( 0, 0, buff1 );
						sprintf( buff1, "A : CW B : CCW", NumberValue );  IOX_CLCD_DisplayString( 1, 0, buff1 );
						if ( stop ) { planTrajec(NumberValue*400); stop = false; }
						break;
					case 'A' :
						sprintf( buff1, "Enter Rev No?:%3d", NumberValue );  IOX_CLCD_DisplayString( 0, 0, buff1 );
						sprintf( buff1, "A : CW B : CCW", NumberValue );  IOX_CLCD_DisplayString( 1, 0, buff1 );
